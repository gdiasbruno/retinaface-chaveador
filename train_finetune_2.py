...
from data import MALF, detection_collate_finetune, preproc_finetune, cfg_mnet, cfg_re50, cfg_re50_finetune_test, cfg_re50_finetune_2
...
parser.add_argument('--num_workers', default=2, type=int, help='Number of workers used in dataloading') # changed default from 4 to 2
...
def train():
    ...
    weight_path = cfg['filename'] + '_Final_test_2'
...
def adjust_learning_rate(optimizer, gamma, epoch, step_index, iteration, epoch_size):
    ...
    if epoch <= warmup_epoch:
        lr = 1e-12 + (initial_lr-1e-12) * iteration / (epoch_size * warmup_epoch)
    else:
